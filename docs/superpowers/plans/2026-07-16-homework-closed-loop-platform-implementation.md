...
| 计划版本 | 1.1 |
| 日期 | 2026-07-17 |
| 状态 | 阶段0—7本地完成；阶段8本地门禁完成、真实环境与部署待凭据 |
...
| 阶段 | 状态 | 已完成 |
|---|---|---|
| 阶段0：工程基线 | 已完成 | 响应式应用壳、iOS 设计令牌、三角色首页、CI、构建与渲染测试 |
| 阶段1：认证与 RLS | 本地完成 | 真实登录边界、一次性邀请、分科授权、撤销即失权、全新 PostgreSQL 多角色集成测试 |
| 阶段2：作业与知识体系 | 已完成 | 家长录入、完整版本修订、知识映射、提交节点、软归档与恢复 |
| 阶段3：日期计划 | 已完成 | 90 分钟日期块、移动/整链移动、拆分、合并、追加、归档恢复、风险和审计 |
| 阶段4：孩子学习动作 | 已完成 | 开始、暂停、继续、完成、不会题号独立保存、断网草稿恢复 |
| 阶段5：家教闭环 | 已完成 | 勾选式批改、订正、独立复做、掌握、提交双确认、撤销与任务重开 |
| 阶段6：证据与看板 | 已完成 | 当前/历史最高掌握、三角色看板、站内通知、周报、导出与校验备份 |
| 阶段7：权威暑期数据 | 已完成 | 200任务块/173作业/23课程日；原课表哈希与五CSV重导入复核 |
| 阶段8：可靠性与上线 | 本地完成，远程待凭据 | 本地发布门禁通过；待真实 Supabase 迁移、四账号/Realtime 验收和私有域名部署 |

未配置 Supabase 时，根页面的角色切换只用于开发预览，不代表生产权限。真实权限以 Supabase Auth、服务端校验和数据库 RLS 三层共同决定。逐项实现与证据见 `docs/release/2026-07-17-acceptance-traceability.md`。
