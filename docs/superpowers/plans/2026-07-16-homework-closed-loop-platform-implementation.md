...
| 阶段 | 状态 | 已完成 |
|---|---|---|
| 阶段0：工程基线 | 已完成 | 响应式应用壳、iOS 设计令牌、三角色首页、测试与构建链路 |
| 纵向闭环预览 | 已完成 | 90 分钟日历、计划调整、孩子学习动作、家教勾选式批改、双确认和三轨状态 |
| 阶段1：认证与 RLS | 基础完成 | 登录界面、Supabase 客户端、家庭/孩子/六科/分科家教/邀请表与 RLS migration |
| 阶段1：真实环境验证 | 验收链路已就绪，待凭据 | 已加入远程迁移和合成账号自动验收；配置 Supabase 项目后执行真实登录、邀请、跨科隔离、撤销即失权与自动清理 |

当前根页面的角色切换用于开发预览，不代表生产权限。真实权限以 Supabase Auth、服务端校验和数据库 RLS 三层共同决定。
