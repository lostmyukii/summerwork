...
begin
  ...
  return jsonb_build_object(
    ...
    'task_knowledge_links', coalesce((select jsonb_agg(to_jsonb(link)) from public.task_knowledge_links link join public.homework_tasks task on task.id = link.task_id where task.student_id = target_student_id), '[]'::jsonb),
    'plan_dependencies', coalesce((select jsonb_agg(to_jsonb(dependency)) from public.plan_dependencies dependency join public.homework_tasks task on task.id = dependency.predecessor_task_id where task.student_id = target_student_id), '[]'::jsonb),
    'plan_changes', coalesce((select jsonb_agg(to_jsonb(change) order by change.created_at) from public.task_plan_changes change join public.homework_tasks task on task.id = change.task_id where task.student_id = target_student_id), '[]'::jsonb),
    ...
    'review_revocations', coalesce((select jsonb_agg(to_jsonb(revocation) order by revocation.revoked_at) from public.task_review_revocations revocation join public.task_review_records review on review.id = revocation.review_id join public.homework_tasks task on task.id = review.task_id where task.student_id = target_student_id), '[]'::jsonb),
    ...
    'correction_revocations', coalesce((select jsonb_agg(to_jsonb(revocation) order by revocation.revoked_at) from public.correction_attempt_revocations revocation join public.correction_attempts correction on correction.id = revocation.correction_attempt_id join public.homework_tasks task on task.id = correction.task_id where task.student_id = target_student_id), '[]'::jsonb),
    'submission_checkpoints', coalesce((select jsonb_agg(to_jsonb(checkpoint) order by checkpoint.created_at) from public.submission_checkpoints checkpoint join public.homeworks homework on homework.id = checkpoint.homework_id where homework.student_id = target_student_id), '[]'::jsonb),
    ...
    'knowledge_nodes', coalesce((select jsonb_agg(to_jsonb(node) order by node.subject_id, node.sort_order, node.display_name) from public.knowledge_nodes node where node.student_id = target_student_id), '[]'::jsonb),
    ...
    'mastery_evidence_revocations', coalesce((select jsonb_agg(to_jsonb(revocation) order by revocation.revoked_at) from public.mastery_evidence_revocations revocation join public.mastery_evidence evidence on evidence.id = revocation.evidence_id where evidence.student_id = target_student_id), '[]'::jsonb),
    ...
    'notifications', coalesce((select jsonb_agg(to_jsonb(notification) order by notification.created_at) from public.notifications notification where notification.student_id = target_student_id), '[]'::jsonb),
